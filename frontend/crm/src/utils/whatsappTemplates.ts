...
/**
 * Format a time string from ISO or date string, or return fallback
 */
...
/**
 * Accurately determines if the booking belongs to Tours & Travels or Self-Drive / Bus Rental
 */
export function getBookingVertical(raw: BookingDataInput | null | undefined): BookingVertical {
  ...
  // Strict Fleet / Rental indicators
  if (
    typeStr === 'fleet' ||
    typeStr === 'rental' ||
    typeStr === 'car' ||
  ...
  // Fallback: check if itemName mentions yatra / tour / darshan
...
/**
 * Normalizes any booking object from CRM / Website into consistent fields
 */
export function extractBookingDetails(raw: BookingDataInput | null | undefined): ExtractedBookingDetails {
  ...
  const pickupDateFormatted = pickupRaw ? formatDate(pickupRaw) : 'Scheduled Date';
  const pickupTimeFormatted = raw.pickupTime || extractTime(pickupRaw, '06:00 AM');

  const dropoffDateFormatted = dropoffRaw ? formatDate(dropoffRaw) : (pickupRaw ? formatDate(pickupRaw) : 'Return Date');
...
/**
 * Replaces all `{tag_name}` placeholders in template text with extracted values
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// 1. TOURS & TRAVELS TEMPLATES (Dedicated to Tour Packages & Chauffeur Trips)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// 2. SELF-DRIVE & BUS RENTAL TEMPLATES (Dedicated to Car Rentals & Bus Fleets)
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Returns strictly the templates belonging to the specific booking vertical
 */
...
/**
 * Auto-suggest the most relevant template based on booking vertical and status
 */
